...
dbCreateTables conn = do
    mapM_ (flip (run conn) [])
        ...
        , "CREATE TABLE grp (id INTEGER PRIMARY KEY, description VARCHAR(1024))"
        , "CREATE TABLE grpmap (group_id INTEGER NOT NULL, data_id INTEGER NOT NULL, UNIQUE(group_id, data_id) ON CONFLICT REPLACE)"
